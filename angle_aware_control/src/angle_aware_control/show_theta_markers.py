...
class ShowPointCloud2d:
    ...
    def __init__(self):
        ...
        x_start = field_grid[0].reshape([-1, 1])
        y_start = field_grid[1].reshape([-1, 1])
        z_start = field_grid[2].reshape([-1, 1])

        now = rospy.Time.now()
        arrow_scale = Vector3(0.001, 0.001, 0.002)
        for i, (x1, y1, z1, x2, y2, z2) in enumerate(zip(x_start, y_start, z_start, self._x_row, self._y_row, self._z_row)):
            marker = Marker()
            marker.header.frame_id = self.WORLD_TF
        ...
        self._pub_markers = rospy.Publisher(
            output_topic, MarkerArray, queue_size=1
        )
    ...
    def publish_markers(self, msg):
        phi = np.array(msg.data).reshape([-1, 1])
        rgba_phi = plt.get_cmap("jet")(phi).squeeze()
        self.create_markers(rgba_phi)
        self._pub_markers.publish(self._marker_array)


    def create_markers(self, colors):
        for i, color in enumerate(colors):
            rgba = ColorRGBA(*color)
            self._marker_array.markers[i].color = rgba
